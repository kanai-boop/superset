
import json
from superset_tools.api_client import SupersetClient

def create_tech_geo_charts():
    """
    Creates charts for the Tech / Geo section of the GA Dashboard.
    """
    client = SupersetClient()
    client.login()

    dataset_id = 34 # v_ga_sessions dataset

    charts_to_create = [
        {
            "chart_name": "GA: Top Regions",
            "viz_type": "bar",
            "params": {
                "groupby": ["geo_region"],
                "metrics": ["Sessions"],
                "x_axis": "geo_region",
                "y_axis": "Sessions",
                "row_limit": 10,
                "rich_tooltip": True,
            }
        },
        {
            "chart_name": "GA: Device Category Breakdown (Pie)",
            "viz_type": "pie",
            "params": {
                "groupby": ["device_category"],
                "metric": "Sessions",
                "show_legend": True,
                "show_labels": True,
            }
        },
        {
            "chart_name": "GA: Device OS Breakdown",
            "viz_type": "pie",
            "params": {
                "groupby": ["device_os"],
                "metric": "Sessions",
                "show_legend": True,
                "show_labels": True,
            }
        },
        {
            "chart_name": "GA: Device Browser Breakdown",
            "viz_type": "pie",
            "params": {
                "groupby": ["device_browser"],
                "metric": "Sessions",
                "show_legend": True,
                "show_labels": True,
            }
        },
    ]

    print(f"Creating {len(charts_to_create)} tech/geo charts for dataset {dataset_id}...")

    for chart_def in charts_to_create:
        chart_def["params"]["datasource"] = f"{dataset_id}__table"
        chart_def["params"]["time_range_endpoints"] = ["inclusive", "exclusive"]

        payload = {
            "slice_name": chart_def["chart_name"],
            "viz_type": chart_def["viz_type"],
            "datasource_id": dataset_id,
            "datasource_type": "table",
            "params": json.dumps(chart_def["params"]),
        }

        try:
            result = client.create_chart(payload)
            new_chart_id = result.get("id")
            print(f"Successfully created chart ''{chart_def['chart_name']}'' with ID: {new_chart_id}")
        except Exception as e:
            print(f"Error creating chart ''{chart_def['chart_name']}'': {e}")

if __name__ == "__main__":
    create_tech_geo_charts()
